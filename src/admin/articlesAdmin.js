...
export default function ArticlesAdmin(){
    ...
    function submit(){
        if(submitAndUpdate === "submit"){
            if(value !== null  && title !== ""){
                ...
                fetch(server+'api/uploadArticles', {
                    method: 'POST',
                    body: formData
                })
                ...
                fetch(server+'api/newArticles', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({
                        namePhoto,title,pointDB
                    })
                })
                    .then(response => response.text())
                    .then(data => {
            ...
            .then(data => {
                setRows(data.map((row,index) => (
                    <TableRow key={row.id}>
                        ...
                        <TableCell><CardImg src={process.env.PUBLIC_URL+"/Articles/"+row.namephoto}></CardImg></TableCell>
                        ...
                        <TableCell>
                            ...
                            <EditIcon onClick={()=>{
                                id=row.id
                                edit(row.namephoto,row.title, row["point"] )}}/>
                        </TableCell>
                    </TableRow>
                )))
            })
    ...
    function deleteF(){
        fetch(server+"api/deleteArticles",{
            headers: {
                'Content-Type': 'application/json',
            },
            method: 'POST',
    ...
    function edit(namePhotoInput,title,point){
        idFrom++
        submitAndUpdate="update"
        document.getElementById("input").focus()
        setButton(<><Button onClick={()=>{clear()}}>Додати нову статтю</Button></>)
    ...
    function editDB(){
        ...
        if( title !== ""){
            ...
            fetch(server+"api/updateArticles",{
                headers: {
                    'Content-Type': 'application/json',
                },
                method: 'POST',
                body:JSON.stringify({
                    id, namePhoto,title, newPhoto,oldPhoto,pointDB
                })
            }).then(response => {
                return response.text();
            }).then(()=>{
            ...
            <Grid item xs={12}>
                <Paper className={classes.paper}>
                    ...
                    <form id="form" style={{width:"100%"}} key={idFrom}>
                        ...
                        <Input id="input" style={{width:"100%",margin:"10px 0px 10px"}}  placeholder={'Посилання ' + 1 } onChange={(e)=>{
                            ...
                            return (
                                <>
                                    ...
                                    <Input
                                        ...
                                        placeholder={'Посилання '+countPoint+' *'}
                                        onChange={(e) => {
