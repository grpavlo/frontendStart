...
export default function Service(){
    ...
    function submit(){
        if(submitAndUpdate === "submit"){
            if(value !== null && url !== ""){
                ...
                fetch(server+'api/uploadService', {
                    method: 'POST',
                    body: formData
                })
            ...
            .then(data => {
                ...
                setRows(data.map((row,index) => (
                    <TableRow key={row.id}>
                        ...
                        <TableCell><CardImg src={process.env.PUBLIC_URL+"/Service/"+row.namephoto}></CardImg></TableCell>
                        ...
                        <TableCell>
                            ...
                            <EditIcon onClick={()=>{
                                id=row.id
                                edit(row.namephoto,row.text,row.url)}}/>
                        </TableCell>
                    </TableRow>
                )))
            })
    ...
    function deleteF(){
        fetch(server+"api/deleteService",{
            headers: {
                'Content-Type': 'application/json',
            },
            method: 'POST',
    ...
    function edit(namePhotoInput,text,url){
        idFrom++
    ...
    function editDB(){
        ...
        if(url !== ""){
            ...
            fetch(server+"api/updateService",{
                headers: {
                    'Content-Type': 'application/json',
                },
                method: 'POST',
                body:JSON.stringify({
                    id, namePhoto, text, url, newPhoto,oldPhoto
                })
            }).then(response => {
                return response.text();
            }).then(()=>{
            ...
            <Grid item xs={12}>
                <Paper className={classes.paper}>
                    ...
                    <form id="form" style={{width:"100%"}} key={idFrom}>
                        ...
                        />
                        <br/>
                        <Input id="input" style={{width:"100%",margin:"10px 0px 10px"}}  placeholder="Текст *" onChange={(e)=>{setText(e.target.value)}} defaultValue={text}/>
                        <br/>
